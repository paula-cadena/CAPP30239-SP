...
def lines_subregion(series):
    ...
    world_bank = world_bank.groupby(['Year', 'Sub-region'], as_index=False)['Value'].mean()
    
    #Lines by subregion
    base = alt.Chart(world_bank).encode(color=alt.Color("Sub-region", legend=None)
                                        ).properties(
...
heat_map_series('Literacy rate, adult total (% of people ages 15 and above)').save('heat_hist.svg')
bars_twoyears('Net migration', 2023).save('bars.svg')
lines_subregion('Ratio of female to male labor force participation rate (%) (modeled ILO estimate)').save('lines.svg')
